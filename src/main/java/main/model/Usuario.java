...
public class Usuario {
    ...
    @Column(nullable = false)
    private String rol; // "ROLE_ALUMNO", "ROLE_ADMIN", "ROLE_TUTOR_EMPRESA", "ROLE_TUTOR_CENTRO"

    private boolean activo = true;

    // Relación inversa con Alumno (si existe)
    @OneToOne(mappedBy = "usuario", cascade = CascadeType.ALL)
    private Alumno alumno;

    // Relación inversa con Administrador (si existe)
    @OneToOne(mappedBy = "usuario", cascade = CascadeType.ALL)
    private Administrador administrador;

    // Relación inversa con TutorEmpresa (si existe)
    @OneToOne(mappedBy = "usuario", cascade = CascadeType.ALL)
    private TutorEmpresa tutorEmpresa;

    // Relación inversa con TutorCentro (si existe)
    @OneToOne(mappedBy = "usuario", cascade = CascadeType.ALL)
